...
def getdata():# csv dosyalarından data okuma ve yazma
    os.chdir('Data')
    if len(os.listdir())==0:
...
def attendance(name): # attendance dosyası için oluşturuldu.
    with open('Attendance.csv', 'r+') as f:
        ...
        if name not in peoplename: # tekrar tekrar yazılmasını engellemek için yazıldı
...
while True : # kemara ile yüzleri eşleştiren sistem
    ret , frame =cam.read()
